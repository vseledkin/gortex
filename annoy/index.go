...
func NewAnnoyIndex(metaFile string, distance Distance, random Random) (AnnoyIndex, error) {

	meta, err := loadMeta(metaFile)
	if err != nil {
		return AnnoyIndex{}, err
	}
	tree := meta.tree
	dim := meta.dim
	K := meta.K
...
func (g AnnoyIndex) MetaFile() string {
	return g.meta.file.Name()
}

func (g *AnnoyIndex) AddItem(key int, w []float32) error {
	args := buildArgs{action: ADD, key: key, w: w, result: make(chan error)}
	g.buildChan <- args
	return <-args.result
}

func (g *AnnoyIndex) RemoveItem(key int) error {
	args := buildArgs{action: DELETE, key: key, result: make(chan error)}
	g.buildChan <- args
	return <-args.result
}

func (g *AnnoyIndex) UpdateItem(key int, w []float32) error {
	args := buildArgs{action: UPDATE, key: key, w: w, result: make(chan error)}
	g.buildChan <- args
	return <-args.result
}

func (g *AnnoyIndex) GetNnsByKey(key, n, searchK int) ([]int, error) {
	m, err := g.nodes.getNodeByKey(key)
	if err != nil || !m.isLeaf() {
		return []int{}, fmt.Errorf("Item with key=%d Not found 1", key)
	}
	return g.GetAllNns(m.v, n, searchK)
}

func (g *AnnoyIndex) GetVectorByKey(key int) ([]float32, error) {
	m, err := g.nodes.getNodeByKey(key)
	if err != nil || !m.isLeaf() {
		return nil, fmt.Errorf("Item with key=%d Not found 2", key)
	}
	return m.v, nil
}

func (g *AnnoyIndex) GetAllNns(v []float32, n, searchK int) ([]int, error) {
	if searchK == -1 {
		searchK = n * g.tree
	}
	...
	for _, root := range g.meta.roots() {
		q.Push(&Queue{priority: math.MaxFloat32, value: root})
	}
	...
	for len(nns) < searchK && q.Len() > 0 {
		...
		if nd.isLeaf() {
			...
			q.Push(&Queue{priority: Min(d, +margin), value: nd.children[1]})
			q.Push(&Queue{priority: Min(d, -margin), value: nd.children[0]})
		}
	}
...
func (g *AnnoyIndex) AddItems(keys []int, ws [][]float32) error {
...
func (g *AnnoyIndex) build(index, root int, n Node) {
	if root == -1 {
		// 最初のノード
		n.parents[index] = -1
		n.save()
...
func (g *AnnoyIndex) remove(root int, node Node) {
	if node.isRoot(root) {
...
func (g AnnoyIndex) findBranchByVector(id int, v []float32) int {
...
func (g *AnnoyIndex) makeTree(root, parent int, ids []int) int {
	if len(ids) == 1 {
...
func (g AnnoyIndex) printTree(root int, node Node, id, tab int) {
	for i := 0; i < tab*2; i++ {
		fmt.Print(" ")
	}
