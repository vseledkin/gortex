...
func (f *File) Find(id int) (Node, error) {
	...
	if node.nDescendants == 1 {
		...
		node.v = bytesToFloat32s(b[f.offsetOfV:])
...
func (f File) nodeToBytes(node Node) []byte {
	...
	if node.isBucket() {
		...
		for i, v := range node.v {
			binary.BigEndian.PutUint32(bytes[offsetOfV+i*8:offsetOfV+i*8+8], math.Float32bits(v))
		}
	}
	return bytes
}
...
func bytesToFloat32s(bytes []byte) []float32 {
	...
	for i := 0; i < size; i++ {
		floats[i] = math.Float32frombits(binary.BigEndian.Uint32(bytes[0:8]))
		bytes = bytes[8:]
	}
	return floats
}
